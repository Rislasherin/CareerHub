...
export default function HRDashboard() {
  ...
  const stats = [
    { 
      ...
      value: loading ? '...' : (data?.stats?.totalCandidates ?? 0).toString(), 
      badge: 'Active', 
      icon: Users, 
      color: 'text-indigo-600', 
      bg: 'bg-indigo-50', 
      border: 'border-indigo-100', 
      sub: loading ? 'Loading...' : `${data?.stats?.totalCandidates ?? 0} candidates linked` 
    },
    { 
      label: 'Interviews Scheduled', 
      value: loading ? '...' : (data?.stats?.interviewsScheduled ?? 0).toString(), 
      badge: 'Upcoming', 
      icon: Calendar, 
      color: 'text-blue-600', 
      bg: 'bg-blue-50', 
      border: 'border-blue-100', 
      sub: loading ? 'Loading...' : `${data?.stats?.interviewsScheduled ?? 0} active sessions` 
    },
    { 
      label: 'Active Jobs', 
      value: loading ? '...' : (data?.stats?.activeJobs ?? 0).toString(), 
      badge: 'Live', 
      icon: Briefcase, 
      color: 'text-amber-600', 
      bg: 'bg-amber-50', 
      border: 'border-amber-100', 
      sub: loading ? 'Loading...' : `${data?.stats?.activeJobs ?? 0} jobs open` 
    },
    { 
      label: 'Offer Letters Sent', 
      value: loading ? '...' : (data?.stats?.offerLettersSent ?? 0).toString(), 
      badge: 'Status', 
      icon: FileText, 
      color: 'text-emerald-600', 
      bg: 'bg-emerald-50', 
      border: 'border-emerald-100', 
      sub: loading ? 'Loading...' : `${data?.stats?.offerLettersSent ?? 0} letters accepted` 
    },
  ];
             ...
             <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                   ...
                   <p className="text-indigo-100 font-medium text-lg max-w-2xl">
                     Manage your complete hiring flow. View candidate funnels, coordinate interview sessions, and scale up your interview panel.
                   </p>
                </div>
             ...
             <GlassCard className="p-10 rounded-[2.5rem] border-slate-100 shadow-sm flex flex-col md:flex-row gap-12">
                ...
                <div className="flex-1 space-y-8">
                   ...
                   <div className="space-y-6">
                      {loading ? (
                        <div className="text-xs font-bold text-slate-400">Loading funnel...</div>
                      ) : (
                        (data?.funnel ?? []).map((f, i) => {
                          ...
                          return (
                            <div key={i} className="space-y-2">
                               ...
                               <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                  <div className={`h-full ${f.color} rounded-full`} style={{ width: `${(f.value / maxValue) * 100}%` }} />
                               </div>
                            </div>
                          );
                        })
