import { IPracticeTTSService } from "../../../application/interfaces/ai-practice/IPracticeTTSService";
...
export class PracticeCartesiaAdapter implements IPracticeTTSService {
  ...
  async *generateAudioStream(text: string): AsyncIterable<Int16Array> {
    ...
    while (attempt <= maxAttempts) {
      ...
      this.handlers.set(contextId, {
         onChunk: (data: { data: string; context_id: string }) => {
            if (!firstAudioReceived) {
              ...
              Logger.info(LogCategory.SYSTEM_INFO, `[PRACTICE_FLOW] CARTESIA_AUDIO_RECEIVED (First chunk)`);
      ...
      try {
        Logger.info(LogCategory.SYSTEM_INFO, `[PRACTICE_FLOW] CARTESIA_TTS_STARTED`);
