export class AnswerEvaluation {
  ...
  private readonly _quality: 'EXCELLENT' | 'GOOD' | 'AVERAGE' | 'POOR' | 'UNACCEPTABLE';
  private readonly _feedback: string;
  private readonly _needsFollowUp: boolean;

  constructor(props: {
    score: number;
    quality: 'EXCELLENT' | 'GOOD' | 'AVERAGE' | 'POOR' | 'UNACCEPTABLE';
    feedback: string;
    ...
    const validQualities = ['EXCELLENT', 'GOOD', 'AVERAGE', 'POOR', 'UNACCEPTABLE'];
  ...
  // Notice: No setters. Value Objects are strictly immutable.
}
