...
function SetupContent() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        const response: any = await apiClient.get(`/auth/interviewer/verify-token/${token}`);
        if (response.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response: any = await apiClient.post(`/auth/interviewer/activate?email=${encodeURIComponent(email || '')}`, 
        { password }, 
