...
const Logo = ({ white = false }: { white?: boolean }) => (
  <div className="flex items-center gap-3 group cursor-pointer">
    <div className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 shadow-lg shadow-indigo-500/30 overflow-hidden group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300">
...
export default function RegisterSelectionPage() {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-500/30 flex flex-col overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className="relative z-10 flex justify-between items-center py-3 px-[5%] lg:px-[10%] border-b border-white/20 bg-white/70 backdrop-blur-xl shadow-sm sticky top-0">
        <Link href="/">
          <Logo />
        </Link>
      ...
      <main className="relative z-10 flex-1 py-8 px-[5%] lg:px-[10%] flex flex-col justify-center">
        <div className="max-w-6xl mx-auto w-full">
          
          <div className="mb-10 text-center flex flex-col items-center">
            <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50/80 border border-indigo-100 text-indigo-700 font-semibold text-[10px] uppercase tracking-widest mb-4 backdrop-blur-sm shadow-sm">
            ...
            <motion.h1 
              ...
              className="text-3xl md:text-5xl lg:text-6xl font-black text-slate-800 mb-4 tracking-tight leading-none"
            >
            ...
            <motion.p 
              ...
              className="text-base md:text-lg text-slate-500 max-w-xl mx-auto font-medium leading-relaxed"
            >
            ...
            {registerRoutes.map((route) => (
              <motion.div key={route.id} variants={fadeInUp} className="h-full">
                <Link href={route.href} className="block h-full group outline-none">
                  <div className="h-full p-6 md:p-8 rounded-3xl bg-white border border-slate-200 hover:border-transparent hover:shadow-[0_20px_60px_-15px_rgba(99,102,241,0.2)] transition-all duration-500 group-hover:-translate-y-1.5 flex flex-col relative overflow-hidden">
                    ...
                    <div className={`relative z-10 w-12 h-12 rounded-xl ${route.bgLight} flex items-center justify-center mb-6 shadow-sm border border-slate-100 group-hover:scale-110 transition-transform duration-500 bg-white`}>
                      <route.icon size={24} className={route.textLight} strokeWidth={2} />
                      <div className="absolute inset-0 rounded-xl bg-white opacity-0 group-hover:opacity-20 transition-opacity"></div>
                    </div>

                    <div className="relative z-10 flex-1">
                      <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-1 block">{route.label}</span>
                      <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-2 group-hover:text-indigo-700 transition-colors tracking-tight">{route.title}</h3>
                      <p className="text-slate-500 text-xs md:text-sm leading-relaxed mb-6">{route.description}</p>
                    </div>

                    <div className={`relative z-10 mt-auto flex items-center gap-2 ${route.textLight} font-bold text-xs md:text-sm`}>
                      Continue <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform duration-300" />
                    </div>
                  </div>
                </Link>
              </motion.div>
