...
export class OpenAIService implements IAIService {
    ...
    async analyzeResume(resumeData: unknown): Promise<IAtsAnalysisResult> {
        try {
            ...
            logger.error("[OpenAI] analyzeResume failed:", msg);
            throw new AppError("OpenAI Resume Analysis failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async autoFixText(text: string, instructions: string): Promise<string> {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] autoFixText failed:", msg);
            throw new AppError("OpenAI Text Auto-Fix failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async extractResumeFromDocument(fileBuffer: Buffer, mimeType: string): Promise<Record<string, unknown>> {
        ...
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] extractResumeFromDocument failed:", msg);
            throw new AppError("OpenAI Document Extraction failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async rewriteEntireResume(resumeData: unknown, targetRole: string): Promise<Record<string, unknown>> {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] rewriteEntireResume failed:", msg);
            throw new AppError("OpenAI Resume Rewrite failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async matchJobDescription(resumeData: unknown, jobDescription: string): Promise<IJobMatchReport> {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] matchJobDescription failed:", msg);
            throw new AppError("OpenAI Job Description Match failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async coachSection(sectionName: string, sectionData: unknown, instructions: string, targetRole: string): Promise<ISectionCoachResult> {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] coachSection failed:", msg);
            throw new AppError("OpenAI Section Coaching failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
    ...
    async generateProfessionalSummary(profileData: unknown): Promise<string> {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            logger.error("[OpenAI] generateProfessionalSummary failed:", msg);
            throw new AppError("OpenAI Summary Generation failed: " + msg, HttpStatus.SERVICE_UNAVAILABLE, ErrorCode.INTERNAL_ERROR);
        }
    }
}