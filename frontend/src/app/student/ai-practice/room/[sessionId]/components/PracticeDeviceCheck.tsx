...
import { Camera, Mic, Sparkles, VideoOff, RefreshCw, ArrowRight, CheckCircle2, ArrowLeft } from 'lucide-react';

interface IPracticeDeviceCheckProps {
...
export const PracticeDeviceCheck: React.FC<IPracticeDeviceCheckProps> = ({
  onProceed,
  isJoining,
  onCancel,
  topic = 'General',
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex flex-col justify-between p-6 md:p-10 font-sans selection:bg-indigo-500 selection:text-white">
      {/* Top Bar */}
      <header className="flex items-center justify-between max-w-6xl mx-auto w-full pb-6 border-b border-slate-900">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white font-black text-lg shadow-lg shadow-indigo-600/30">
            C
          </div>
          <div>
            <span className="font-extrabold text-sm tracking-tight text-white flex items-center gap-1.5">
              CareerHub <span className="text-slate-600">•</span> <span className="text-indigo-400 font-bold">AI Practice</span>
            </span>
                ...
                <div className="flex flex-col items-center gap-4 text-slate-400 p-8 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 max-w-xs">
                      {status.error || 'Please grant camera & microphone permissions to begin practice.'}
                    </p>
                  </div>
                  <button
                    onClick={retry}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className={`p-4 rounded-2xl border flex items-center gap-3.5 transition-all ${
                ...
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                  status.cameraPermission === 'granted' ? 'bg-emerald-900/50 text-emerald-400' : 'bg-indigo-900/50 text-indigo-400'
                }`}>
                  <Camera size={18} />
                </div>
                <div>
              ...
              <div className={`p-4 rounded-2xl border flex items-center gap-3.5 transition-all ${
                ...
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                  status.microphonePermission === 'granted' ? 'bg-emerald-900/50 text-emerald-400' : 'bg-indigo-900/50 text-indigo-400'
                }`}>
                  <Mic size={18} />
                </div>
                <div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">
                Get ready for your practice round
              </h2>
            ...
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Topic</p>
                  <p className="text-sm font-extrabold text-white">{topic}</p>
                </div>
              </div>
              <span className="font-mono text-[10px] font-black text-indigo-400 bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800 uppercase tracking-widest">
                {difficulty}
              </span>
            </div>
            ...
            <button
              ...
              className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 text-white font-extrabold text-sm rounded-2xl transition-all shadow-xl shadow-indigo-600/25 disabled:shadow-none flex items-center justify-center gap-2.5 cursor-pointer disabled:cursor-not-allowed"
            >
              {isJoining ? (
               <span className="flex items-center gap-2">
                 <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                 Entering Practice Room...
               </span>
              ) : (
                <>
                  Join Practice Interview <ArrowRight size={18} />
                </>
              )}
            </button>
          </div>
      ...
      <footer className="text-center text-slate-600 text-[11px] font-semibold pt-4 border-t border-slate-900 max-w-6xl mx-auto w-full">
        Powered by CareerHub Real-time AI Evaluation Pipeline &bull; Practice Session
      </footer>
    </div>
  );
};
