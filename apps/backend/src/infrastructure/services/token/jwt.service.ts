...
export class JwtService implements IJwtService {
  private accessOptions: SignOptions = { expiresIn: JWT_ACCESS_EXPIRES_IN as SignOptions["expiresIn"] };
  private refreshOptions: SignOptions = { expiresIn: JWT_REFRESH_EXPIRES_IN as SignOptions["expiresIn"] };

  signAccessToken(payload: JwtPayload): string {
    return jwt.sign(payload, JWT_ACCESS_SECRET, this.accessOptions);
  }

  signRefreshToken(payload: JwtPayload): string {
    return jwt.sign(payload, JWT_REFRESH_SECRET, this.refreshOptions);
  }
